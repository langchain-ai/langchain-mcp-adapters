...
class TestSecurityLimits:
    """Test security limit enforcement (size, properties)."""

    async def test_schema_size_within_limit(self):
        """Test that schemas within size limit are accepted."""
        handler = DefaultElicitationHandler(max_schema_size=1000)

        # Small schema (well under 1000 bytes)
        schema = {
            "type": "object",
            "properties": {
                "field1": {"type": "string"},
        ...
        response = await handler.handle_elicitation("Test", schema, "test_server")
        assert response["action"] == "accept"

    async def test_schema_size_exceeds_limit(self):
        """Test that schemas exceeding size limit are declined."""
        handler = DefaultElicitationHandler(max_schema_size=100)

        # Large schema (over 100 bytes)
        ...
        response = await handler.handle_elicitation("Test", schema, "test_server")
        assert response["action"] == "decline"
        assert "size" in response["reason"].lower()

    async def test_property_count_within_limit(self):
        """Test that schemas with properties within limit are accepted."""
        handler = DefaultElicitationHandler(max_properties=10)

        # Schema with 10 properties (at limit)
    ...
    async def test_property_count_exceeds_limit(self):
        """Test that schemas exceeding property count are declined."""
        handler = DefaultElicitationHandler(max_properties=5)

        # Schema with 10 properties (exceeds limit of 5)
        properties = {f"field_{i}": {"type": "string"} for i in range(10)}
        schema = {
            "type": "object",
            "properties": properties
        ...
        response = await handler.handle_elicitation("Test", schema, "test_server")
        assert response["action"] == "decline"
        assert "properties" in response["reason"].lower()