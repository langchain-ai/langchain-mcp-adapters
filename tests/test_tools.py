...
async def test_convert_mcp_tool_to_langchain_tool():
    ...
    session.call_tool.assert_called_once_with(
        ...
        # 2026-07-28 may answer with an InputRequiredResult instead of the tool
        # result; every interactive call opts in so the driver can resolve it.
...
async def test_load_mcp_tools():
    ...
    async def mock_call_tool(tool_name, arguments, progress_callback=None, **kwargs):
        if tool_name == "tool1":
