...
async def test_tool_call_with_meta():
    """Test that meta parameter is passed to session.call_tool."""
    tool_input_schema = {
        "properties": {
            "param1": {"title": "Param1", "type": "string"},
        },
    ...
    # Create tool with interceptor that adds meta
    ...
    # Verify call_tool was called with meta
    session.call_tool.assert_called_once_with(
        "test_tool",
        {"param1": "test"},
...
async def test_tool_call_with_meta_via_interceptor_override():
    """Test that interceptors can override meta in the request."""
    tool_input_schema = {
        "properties": {
            "param1": {"title": "Param1", "type": "string"},
        },
    ...
    async def add_meta_interceptor(
        ...
        new_request = request.override(meta={"source": "interceptor1"})
        return await handler(new_request)
    ...
    async def modify_meta_interceptor(
        ...
        existing_meta = request.meta or {}
        modified_meta = {**existing_meta, "modified_by": "interceptor2"}
        new_request = request.override(meta=modified_meta)
        return await handler(new_request)
...
async def test_tool_call_without_meta():
    """Test that tool calls work without meta (passes None)."""
    tool_input_schema = {
        "properties": {
            "param1": {"title": "Param1", "type": "string"},
        },
